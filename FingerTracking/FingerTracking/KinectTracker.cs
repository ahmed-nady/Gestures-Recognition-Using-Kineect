...
namespace FingerTracking
{
    ...
    {
        PointFT p1 = new PointFT();
        PointFT p2 = new PointFT();
        //$N
        ...
        public void depthFrameReady(object sender, DepthImageFrameReadyEventArgs e)
        {
            ...
            for (int i = 0; i < hands.Count; ++i)
            {
                ...
                for (int j = 0; j < hands[i].fingertips.Count; ++j)
                {
                    if (hands[i].fingertips[j].X != -1)
                    {
                        ...
                        TargetsListofPoints.Add(new PointR((double)hands[i].fingertips[j].X,(double) hands[i].fingertips[j].Y, Environment.TickCount));
            ...
            //if (TargetsListofPoints.Count > 40)
            //{
            ...
            //    double dist = PointFT.distanceEuclidean(p1, p2);
            //    if (dist < 10)
            //        recognizeGestureUsingNRecognizer();
        ...
        private void recognizeGestureUsingNRecognizer()
        {
            ...
            if (result.Score > .7)
            {
                if (result.Name == "TranslateRightLine")
                {
                ...
                else if (result.Name == "ZoomInCaret")
                {
                    controlModel("Zoom", "%{UP 10}");
                }
                else if (result.Name == "ZoomOutCaret")
                {
                    controlModel("Zoom", "%{DOWN 10}");
                }
                else if (result.Name == "RotateLeftCircle")
                {
                    controlModel("Rotate", "{LEFT 10}");
                }
                else if (result.Name == "RotateRightPI")
                {
                    controlModel("Rotate", "{RIGHT 10}");
                }
                else if (result.Name == "ZoomIn2Fingers")
                {
                    controlModel("Zoom", "%{UP 10}");
                }
                else if (result.Name == "ZoomOut2Fingers")
                {
                    controlModel("Zoom", "%{DOWN 10}");
                }
